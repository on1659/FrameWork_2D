...
public:
	...
	*/
		...
		float Opacity = 255.0f, float ThumbLength = 0.f, float thickness = 5.0f)
	{
		...
		m_iOpacity = static_cast<int>(Opacity);
		RenewalThumbRect();
	...
	*/
	...
	{
		LONG lPos = ((m_scType == SCRType::SCR_H) ? pt.x - m_ptWorldPosition.x : pt.y - m_ptWorldPosition.y);
		float pos = static_cast<float>(lPos);
	...
	*/
	virtual void Draw(HDC hDC, RECT DrawAreaSize, COLORREF color = RGB(0,0,0))
	{
		...
		RoundRect(LayDC, m_rcScroll.left, m_rcScroll.top, m_rcScroll.right, m_rcScroll.bottom, static_cast<int>(m_flThickness * 0.5f), static_cast<int>(m_flThickness * 0.5f));

		SetDCPenColor(LayDC, color);
		SetDCBrushColor(LayDC, color);
		SelectObject(LayDC, (HBRUSH)GetStockObject(DC_BRUSH));
		SelectObject(LayDC, (HPEN)GetStockObject(DC_PEN));
		RoundRect(LayDC, m_rcThumb.left, m_rcThumb.top, m_rcThumb.right, m_rcThumb.bottom, static_cast<int>(m_flThickness * 0.5f), static_cast<int>(m_flThickness * 0.5f));
